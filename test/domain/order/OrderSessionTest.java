...
public class OrderSessionTest {
	...
	public void testGetCarModels() {
		...
		Mockito.when(mockMan.getVehicleModels()).thenReturn(models);
		assertTrue(session.getVehicleModels().contains(mockModel));
		assertTrue(session.getVehicleModels().size() == 1);
	}
